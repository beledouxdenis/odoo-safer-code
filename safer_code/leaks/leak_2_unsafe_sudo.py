...
class Me(Controller):
    ...
    def me(self, **post):
        ...
        """
        partner = request.env.user.partner_id  # nosemgrep: requests-in-models
        if post:
            partner.sudo().write(post)
            return request.redirect("/me")  # nosemgrep: requests-in-models
        values = {
            "partner": partner,
        }
        return request.render("safer_code.me", values)  # nosemgrep: requests-in-models
...
class MyMessage(Controller):
    ...
    def my_last_messages(self, domain=None):
        ...
        force_domain = [
            ("model", "=", "res.partner"),
            ("res_id", "=", request.env.user.partner_id.id),  # nosemgrep: requests-in-models
        ]
        domain = (domain or []) + force_domain
        return (
            request.env["mail.message"]  # nosemgrep: requests-in-models
            .sudo()
            .search_read(domain, ["subject", "body"], limit=10)
        )
